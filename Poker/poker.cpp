...
private:
	...
	int handPoints[6];
	int bestHand[6][3];
	enum Decision{FLOP = 1, CHECK, BET_or_CALL};
	int playerIndex = 4;
	...
	void takeBets()
	{
		...
		for (int k = bind + 1; k < bind + 7; k++)
		{
			...
			if (k % players_count == 4 && players[playerIndex].round)
			{
				if (betOn)
				{
				...
				else
				{
					...
					else
					{
                            ...
                            cin >> bet;
                            while (bet > players[playerIndex].money || bet < 1)
                            {
                            ...
                            pot += bet;
                            players[playerIndex].money -= bet;
                            betOn = bet;
                            players[playerIndex].goodToGo = 1;

                            cout << "\t+ " << players[playerIndex].name << " bets " << bet << "$\n";
					}
				}
			}
			/* computers actions */
